# Zenith Capital Group — Prospect Pipeline

This file tracks prospective customers, outreach status, public business contact information, conversation history, and next actions for Zenith's initial service-sales effort.

> **Public-repository rule:** Keep this file limited to public business contact information and professional, sanitized notes. Do not record confidential customer information, private personal contact details, credentials, pricing received in confidence, or anything a prospect would reasonably expect to remain private.

## Status Legend

- **Researching** — identified but not yet ready for outreach.
- **Ready** — contact path and outreach angle identified.
- **Contacted** — outreach sent or attempted.
- **Connected** — connection accepted or two-way contact established.
- **Discovery** — active conversation about workflow/problems.
- **Proposal** — paid work proposed.
- **Won** — paid engagement accepted.
- **Closed / Not Now** — not pursuing currently.

---

## Prospect 001 — Tipton Machine & Fabrication, LLC

**Status:** Contacted  
**Priority:** A  
**Location:** 466 Melanie Ln, Munford, TN 38058  
**Business phone:** 901-609-5089  
**Business type:** Custom manufacturing / machine shop / fabrication  
**Website:** http://tiptonmachine.com/  

### Primary Contact

**Paul Leister**  
**Role:** Tipton Machine & Fabrication, LLC; owner-level target for outreach  
**LinkedIn:** https://www.linkedin.com/in/paul-leister-06a01bb3  

### Why Zenith Is Interested

- Local Tipton County manufacturer.
- Custom machining, welding, sheet-metal fabrication, and design/development create plausible operational workflows around quoting, jobs, materials, scheduling, status tracking, and reporting.
- Small/local enough that direct decision-maker access may be possible.
- Strong fit with Zach's manufacturing background and prior inventory/database work.

### Outreach History

**2026-08-13 — LinkedIn connection request sent**

Connection note used:

> Hey Paul, I'm local to Tipton County and have a manufacturing background. I built an inventory/database tool around problems I ran into on the shop floor, and I'm now starting a software/automation business focused on helping local manufacturers. Thought I'd connect.

### Conversation Notes

- No response yet.

### Next Action

- Wait for LinkedIn connection acceptance/reply.
- If accepted, ask a low-pressure discovery question about repetitive/manual work rather than pitching a predetermined software product.
- Potential discovery areas: quoting, job tracking, material tracking, reporting, scheduling, customer/order status, and repetitive data entry.

### Public Sources

- LinkedIn: https://www.linkedin.com/in/paul-leister-06a01bb3
- Business listing: https://www.yellowpages.com/munford-tn/mip/tipton-machine-fabrication-llc-505261265

---

## Prospect 002 — Edward's Sheet Metal Works, Inc.

**Status:** Contacted  
**Priority:** A  
**Location:** 298 Wilkinsville Rd, Drummonds, TN 38023  
**Business phone:** 901-835-4686  
**Fax:** 901-835-4081  
**Business type:** Custom sheet-metal fabrication / industrial metalwork  
**Website:** https://edwardssheetmetalworks.com/  

### Primary Contact

**Decision-maker not yet verified.**

A third-party D&B listing names **James E. Brashier Sr.** as a contact, but that listing is marked unverified. Do not address outreach to him as a confirmed owner/manager until independently verified.

### Why Zenith Is Interested

- Tipton County fabrication business with a workflow profile close to Zach's prior manufacturing experience.
- Publicly described work includes custom fabrication and industrial air/material-handling ducting and related metalwork.
- Plausible discovery areas include material tracking, estimating/quoting, job status, reporting, production handoffs, shipping, and repetitive administrative work.

### Outreach History

**2026-08-13 — Website contact form submitted**

Message used:

> Hi, I'm Zach. I'm local to Tipton County and have a manufacturing background. I previously built an inventory/database tool around material tracking on the shop floor, and I'm starting a local software/automation business. Who would be the best person to speak with about operations or process improvement?

### Conversation Notes

- No response yet.
- Initial outreach intentionally asks for the correct operations/process-improvement contact rather than pitching a predetermined software package.

### Next Action

- Wait for a reply identifying the appropriate decision-maker.
- If there is no response, consider a short follow-up through the website or a phone call later; the first goal remains identifying the right person.
- If a decision-maker responds, move to a low-pressure discovery question about repetitive/manual workflows.

### Public Sources

- Official website: https://edwardssheetmetalworks.com/
- Business listing: https://www.yellowpages.com/drummonds-tn/mip/edwards-sheet-metal-works-inc-534869135

---

## Prospect 003 — Thetford Industrial Solutions / B&J Machine Works

**Status:** Contacted  
**Priority:** A-  
**Location:** 1495 Highway 51 N, Covington, TN 38019  
**Public business phone:** 901-296-5188  
**Business type:** Machine shop / custom machining / industrial fabrication and services  
**Website:** No Tennessee official website confidently verified; do not use the similarly named Texas B&J Machine Works site for outreach.  

### Primary Contact

**Michael Dubeck**  
**Role:** Operations Manager, Thetford Industrial Solutions LLC  
**LinkedIn:** https://www.linkedin.com/in/michael-dubeck-29597222  

### Secondary Contact

**Tommy Thetford**  
**Role:** Owner/President-level contact; Thetford Industrial Solutions / B&J Machine Works  
**LinkedIn:** https://www.linkedin.com/in/tommy-thetford-3551b1157  

### Why Zenith Is Interested

- Michael Dubeck is directly aligned with the type of person Zenith wants to learn from: manufacturing operations, process improvement, documentation, shipping/receiving, Excel, plant management, and production experience.
- B&J Machine Works is publicly listed as a Covington machine shop supporting CNC, prototype/development machining, production runs, lathe/mill work, and related manufacturing services.
- Plausible discovery areas include job tracking, estimating handoffs, production status, materials, shipping/receiving, documentation, and repetitive reporting.

### Outreach History

**2026-08-13 — LinkedIn connection request sent to Michael Dubeck**

Connection note used:

> Hey Michael, I'm local to Tipton County with a manufacturing background. I previously built an inventory/database tool around shop-floor material tracking, and I'm now starting a software/automation business focused on local industrial operations. Thought I'd connect.

### Conversation Notes

- No response yet.

### Next Action

- Wait for Michael to accept or reply.
- If he accepts, ask one focused discovery question about manual/repetitive operational work before discussing services.
- Keep Tommy Thetford as a secondary route if Michael does not respond after an appropriate follow-up interval.

### Public Sources

- Michael Dubeck LinkedIn: https://www.linkedin.com/in/michael-dubeck-29597222
- Tommy Thetford LinkedIn: https://www.linkedin.com/in/tommy-thetford-3551b1157
- Covington B&J listing: https://www.mapquest.com/us/tennessee/b-j-machine-works-380670670

---

## Prospect 004 — Rose Fabricating

**Status:** Ready  
**Priority:** A-  
**Location:** 230 Industrial Road North, Covington, TN 38019  
**Business phone:** 901-476-2202  
**General email:** info@rosefabricating.com  
**Business type:** Steel / custom industrial fabrication  
**Website:** https://rosefabricating.com/  

### Primary Contact

**Mike Scott**  
**Role:** General Manager  
**Public business email:** mscott@rosefabricating.com  

### Secondary Contacts

**Tim Rowland** — Director of Sales & Business Development — trowland@rosefabricating.com  
**Gary Webb** — Director of Engineering and Projects — gwebb@rosefabricating.com  

### Why Zenith Is Interested

- Long-running Covington fabrication company with complex project, engineering, production, quality-control, and reporting workflows.
- Publicly describes project-manager updates, shop-detail drawings, project manuals, QC checkpoints, and automated fabrication equipment.
- Larger and likely more process-mature than the first three prospects, making this a useful test of whether Zenith can find smaller workflow gaps inside an established operation.
- Plausible discovery areas include information handoffs, reporting, project status, data entry, material visibility, and repetitive administrative coordination.

### Outreach History

- Not contacted yet.

### Conversation Notes

- None yet.

### Next Action

- Send a short personalized email to Mike Scott.
- Lead with local manufacturing experience and curiosity about operational bottlenecks rather than claiming Rose has a specific software problem.
- Ask whether there are repetitive reporting, tracking, data-entry, or information-handoff processes that still consume disproportionate staff time.

### Public Sources

- Official contact/team page: https://rosefabricating.com/contact/
- Official capabilities page: https://rosefabricating.com/capabilities/

---

## Pipeline Metrics

| Metric | Count |
|---|---:|
| Prospects researched | 4 |
| Ready for outreach | 1 |
| Contacted | 3 |
| Connected | 0 |
| Discovery conversations | 0 |
| Proposals sent | 0 |
| Customers won | 0 |
| Revenue | $0 |

## Operating Rule

The objective is not to accumulate names. Move prospects through the pipeline by learning about real operational pain, asking useful questions, and proposing small paid improvements only when a genuine problem is identified.
